...
serve(async (req) => {
  ...
  try {
    ...
    // MT11: Validate user has owner/admin role before allowing cancellation
    const roleCheck = await canManageSubscription(supabaseClient, user.id, profile.organization_id);
    if (!roleCheck.isValid) {
      finalLog.warn("Unauthorized cancellation attempt", { role: roleCheck.role, error: roleCheck.error });
