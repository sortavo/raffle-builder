-- =====================================================
-- FIX: Prioritize direct ticket_index lookup
--
-- The previous version checked calculated index first,
-- but test data stores actual number as ticket_index.
-- This version checks direct lookup FIRST.
-- =====================================================
...
BEGIN
  ...
  IF v_search_num IS NOT NULL AND v_search_num > 0 AND v_search_num <= v_total_tickets THEN
    -- Strategy 1: Direct lookup where ticket_index = search_num
    -- (handles cases where ticket_index stores actual number)
    IF EXISTS (
    ...
    -- Not found in reservations - return as available
    RETURN QUERY
    SELECT
      v_search_num,
      LPAD(v_search_num::TEXT, GREATEST(v_min_digits, LENGTH(v_total_tickets::TEXT)), '0'),
  ...
  FROM (
    ...
    FROM generate_series(0, LEAST(v_total_tickets - 1, 100000)) AS gs(idx)
